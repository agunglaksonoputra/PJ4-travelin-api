...
exports.getTransactionSummary = async ({ vehicleId } = {}) => {
  ...
  return Transaction.findAll({
    attributes: ["status", [sequelize.fn("COUNT", sequelize.col("id")), "trip_count"], [sequelize.fn("COALESCE", sequelize.fn("SUM", sequelize.col("total_cost")), 0), "total_amount"]],
    where,
    group: ["status"],
    order: [["status", "ASC"]],
  });
...
exports.setTransactionPaymentPlan = async ({ transactionId, data, actorUserId, transaction: outerTransaction }) => {
  ...
  return runInTransaction(outerTransaction, async (transaction) => {
    ...
    const outstandingAmount = outstandingRaw > 0 ? Math.round(outstandingRaw * 100) / 100 : 0;
...
const createInitialTransactionPaymentOnStatusChange = async ({ transactionRecord, previousStatus, actorUserId, transaction }) => {
  ...
  const existingPayments = await TransactionPayment.count({ where: { transaction_id: transactionRecord.id }, transaction });
